...
const defineStepper = <const Steps extends Stepperize.Step[]>(
  ...
  const { Scoped, useStepper, steps: definedSteps } = Stepperize.defineStepper(...steps);
  ...
  const StepperContainer = ({
    ...
    return (
      <div className={cn("w-full", className)} date-component="stepper" {...props}>
  ...
  return {
    ...
    Stepper: {
      ...
      Provider: ({
        ...
        return (
          <StepperContext.Provider value={{ labelOrientation, tracking, variant }}>
      ...
      Step: ({ children, className, icon, ...props }) => {
        ...
        if (variant === "line") {
          return (
            <li className={cn(className)} data-state={dataState} date-component="stepper-step">
              <button
